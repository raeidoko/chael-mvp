...
uploadPhoto: async (imageDataUrl) => {
  ...
  try {
    ...
    if (authUser) {
      ...
      if (uploadError) {
        ...
        // replace the large base64 preview with the small real URL now that we have it
  ...
  const { error: saveError } = await supabase.from("assessments").insert({
    ...
    photo_path: filePath,
    questionnaire: { source: "photo-only", note: "Initial photo analysis before questionnaire" },
    result: result,
    ...
    pih_present: result.explanation?.toLowerCase().includes("pih") || result.explanation?.toLowerCase().includes("hyperpigmentation") || false,
  });
